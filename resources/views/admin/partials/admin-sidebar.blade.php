                        ...
                        @if (!isset($item['can']) || auth()->user()?->can($item['can']))
                            <li>
                                <a href="{{ route($item['route']) }}"
                                   class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs($item['route'] . '*') ? 'bg-gray-50 dark:bg-gray-900 text-indigo-600 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-50 dark:hover:bg-gray-900' }}">
                                    @if (isset($item['icon']))
